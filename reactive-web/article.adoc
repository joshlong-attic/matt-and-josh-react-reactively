...
By default, our Spring Boot application would start up with a running web server, MongoDB support, and so much more. We only need the machinery related to  MongoDB and our `ProfileService`, in isolation. We'll use the  `@DataMongoTest` to tell Spring Boot to autoconfigure all the things that could be implied in our MongoDB logic, while ignoring things like the web server and runtime. This results in focused, fasted test code that gets the benefit of being easier to reproduce.

Spring Boot goes through and runs auto-configurations when the application starts.
